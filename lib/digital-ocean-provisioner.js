...
_.extend(DigitalOceanProvisioner.prototype, /** @lends DigitalOceanProvisioner.prototype */ {
    ...
    'copyFolder': function(instance_id, source, dest, cb) {
        ...
        this.api.dropletGet(instance_id, function(err, instance) {
            ...
            var rsync_cmd = _.sprintf("rsync -avz --delete -e 'ssh -i %s -o StrictHostKeyChecking=no' %s root@%s:%s", self._options.private_ssh_key, source, instance.ip_address, dest);
    ...
    '_copyInstanceScript': function(ip_address, script, cb) {
        ...
        var copy_cmd = _.sprintf('scp -i %s -o StrictHostKeyChecking=no -q %s root@%s:%s', this._options.private_ssh_key, this._getScriptPath(script), ip_address, target_path);
    ...
    '_executeExistingInstanceScript': function(ip_address, script_path, cb) {
        var script_cmd = _.sprintf('ssh root@%s -i %s -o StrictHostKeyChecking=no -q "chmod +x %s; %s"', ip_address, this._options.private_ssh_key, script_path, script_path),
            self = this;
    ...
    'runInstanceCommand': function(instance_id, cmd, cb) {
        var cmd = _.sprintf('ssh root@%s -i %s -o StrictHostKeyChecking=no -q "%s"', ip_address, this._options.private_ssh_key, cmd),
            self = this;
        this.api.dropletGet(instance_id, function(err, instance) {
            if (err) {
                return cb(err);
