...
_.extend(DigitalOceanProvisioner.prototype, /** @lends DigitalOceanProvisioner.prototype */ {
    ...
    'copyFolder': function(instance_id, source, dest, cb) {
        ...
        this.api.dropletGet(instance_id, function(err, instance) {
            ...
            var rsync_cmd = _.sprintf("rsync -avz --delete -e 'ssh -i %s -o StrictHostKeyChecking=no -o GSSAPIAuthentication=no' %s root@%s:%s", self._options.private_ssh_key, source, instance.private_ip_address, dest);
    ...
    'executeInstanceScript': function(instance_id, script, cb) {
        ...
        var droplet = this.api.dropletGet(instance_id, function(err, instance) {
            ...
            var tasks = [
                function(cb2) {
                    self._copyInstanceScript(instance.private_ip_address, script, cb2);
                },
                function(result, cb3) {
                    self._executeExistingInstanceScript(result.private_ip_address, result.target_path, cb3);
                }
            ];
    ...
    'runInstanceCommand': function(instance_id, cmd, cb) {
        ...
        this.api.dropletGet(instance_id, function(err, instance) {
            ...
            script_cmd = _.sprintf('ssh root@%s -i %s -o StrictHostKeyChecking=no -o GSSAPIAuthentication=no -q "%s"', instance.private_ip_address, self._options.private_ssh_key, cmd),
            self._log('info', 'Running command `' + script_cmd + '` against ip_address: ' + instance.private_ip_address);
            ...
            var kopy = function() {
                ...
                shell.exec(script_cmd, {
                    ...
                    if (code !== 0) {
                        if (attempts > 10) {
                            self._log('error', 'Error running remote instance command.', {
                        ...
                        self._log('info', 'Execution of command `' + script_cmd + '` against ip_address ' + instance.private_ip_address + ' succeeded');
                        cb(null, output);
                    }
                });
            };
            ...
            self._log('info', 'Droplet created', droplet);
                ...
                self.api.eventGet(droplet.event_id, function(err, data) {
                    ...
                    if (data.action_status === 'done') {
                        ...
                        clearInterval(interval);
                        tcpPortUsed.waitUntilUsedOnHost(22, droplet.private_ip_address, 1000, 240000).then(function() {
                            setTimeout(function() {
    ...
    'listDroplets': function() {
        this.api.dropletGetAll(function(err, droplets) {
            ...
            _.each(droplets, function(droplet) {
                table.push([droplet.id, droplet.name, droplet.image_id, droplet.size_id, droplet.region_id, droplet.backups_active, droplet.private_ip_address, droplet.private_ip_address, droplet.locked, droplet.status, droplet.created_at]);
            });
